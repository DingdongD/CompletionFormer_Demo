...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train DySPN HW-aligned model with CSPN NYU h5 data")
    ...
    parser.add_argument("--save-dir", default="/workspace/CSPN/cspn_pytorch/output/agentflow_dyspn_hw128_resnet18")
    ...
    parser.add_argument("--num-neighbor", type=int, default=5)
    parser.add_argument("--network", default="resnet18", choices=["resnet18", "resnet34"])
    parser.add_argument("--pred-init-loss-weight", type=float, default=0.35)
    parser.add_argument("--offset-aff-loss-weight", type=float, default=0.0)
    parser.add_argument("--resume", default="")
    args = parser.parse_args()

    import_remote_modules(Path(args.repo_root))
    from dyspn_test.dyspn_hw_aligned import DySPNHWAlignedModel, DySPNHWConfig
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for step, sample in enumerate(train_loader):
            ...
            if args.offset_aff_loss_weight > 0:
                loss = loss + args.offset_aff_loss_weight * out["offset_aff"].abs().mean()
