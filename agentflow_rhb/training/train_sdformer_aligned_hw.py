#!/usr/bin/env python3
"""Train SDFormerAlignedHW128 on the CSPN NYU h5 dataset layout.

This is the remote-training entry point for the retrain-required SDFormer HW
variant.  It uses the same NYU datalist convention as the CSPN/DySPN aligned
training scripts:

  sample["rgbd"]  -> [B,4,H,W], first 3 channels RGB, fourth sparse depth
  sample["depth"] -> dense target depth
"""
...
def add_import_roots(repo_root: Path, portable_runtime: Path) -> None:
    sys.path.insert(0, str(repo_root))
    sys.path.insert(0, str(repo_root / "models"))
    sys.path.insert(0, str(portable_runtime))
...
def evaluate(model: nn.Module, loader: DataLoader, args: argparse.Namespace, device: torch.device) -> dict[str, float]:
    model.eval()
    total_l1 = 0.0
    total_rmse = 0.0
    count = 0
    with torch.no_grad():
        for sample in loader:
            ...
            pred = model({"rgb": rgb, "dep": sparse})["pred"]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Train SDFormer HW128 aligned model")
    ...
    parser.add_argument("--portable-runtime", default=str(Path(__file__).resolve().parents[2]))
    ...
    parser.add_argument("--save-dir", default="/workspace/CSPN/cspn_pytorch/output/agentflow_sdformer_aligned_hw128")
    ...
    parser.add_argument("--scheduler-step", type=int, default=35)
    parser.add_argument("--scheduler-gamma", type=float, default=0.5)
    parser.add_argument("--n-sample", type=int, default=500)
    parser.add_argument("--subset", type=int, default=0)
    parser.add_argument("--val-subset", type=int, default=512)
    parser.add_argument("--seed", type=int, default=7240)
    parser.add_argument("--data-parallel", action="store_true")
    parser.add_argument("--resume", default="")
    args = parser.parse_args()

    add_import_roots(Path(args.repo_root), Path(args.portable_runtime))
    from agentflow_rhb.training.sdformer_aligned_hw.sdformer_aligned_hw import (  # pylint: disable=import-error
    ...
    model = SDFormerAlignedHW128(SDFormerHW128Config(lr=args.lr)).to(device)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for step, sample in enumerate(train_loader):
            ...
            running += loss.item()
            if step % 50 == 0:
                print(f"epoch={epoch} step={step}/{len(train_loader)} loss={loss.item():.5f}", flush=True)
        if scheduler is not None:
            scheduler.step()
        metrics = evaluate(model, val_loader, args, device)
        metrics.update({"train_loss": running / max(len(train_loader), 1), "epoch_sec": time.time() - t0, "best_l1": best_l1})
        ...
        line = " ".join(f"{k}={v:.6f}" if isinstance(v, float) else f"{k}={v}" for k, v in {"epoch": epoch, **metrics}.items())
