...
BOARD_WORK="${BOARD_WORK:-/home/root/workspace/demo_vp_xj/packers/rhb_auto_nlspn_val32_headpost_20260714}"
LIMIT="${LIMIT:-32}"
START="${START:-0}"
...
OUT_DIR="$WORK/nlspn_val32_headpost_qdq_outputs"
LOG_DIR="$REPORTS/nlspn_val32_headpost_qdq_logs"
SUMMARY="$REPORTS/nlspn_val32_headpost_qdq_summary.csv"
...
AFFINES="$WORK/nlspn_host_affines_ckpt_final.npz"
...
sshpass -p "$BOARD_PASS" scp "${ssh_opts[@]}" "$RUNNER" "$AFFINES" "$BOARD:$BOARD_WORK/"
...
for ((idx=START; idx<end; idx++)); do
  ...
  board_out="${sample}_headpost_qdq.npz"
  log="$LOG_DIR/${sample}_headpost_qdq.log"
  test -f "$feature"
  ...
  sshpass -p "$BOARD_PASS" ssh "${ssh_opts[@]}" "$BOARD" \
    "cd '$BOARD_WORK' && python3 nlspn_hw_board_pipeline_runner.py packer_nlspn_final_dec5_dec4_20260714 \
      ...
      --host-affines-npz nlspn_host_affines_ckpt_final.npz \
      --host-post-prefix pred_init \
      --host-post-prefix guidance \
